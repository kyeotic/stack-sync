...
pub fn sync_dry_run(config: &Config, client: &PortainerClient) -> Result<()> {
    println!("{BOLD}{CYAN}[dry-run]{RESET} Previewing sync for stack '{BOLD}{}{RESET}'", config.name);
    ...
    println!("{BOLD}{CYAN}[dry-run]{RESET} Compose file: {} {DIM}({} bytes){RESET}", compose_path.display(), compose_content.len());
    if let Some(env_path) = config.env_path() {
        println!("{BOLD}{CYAN}[dry-run]{RESET} Env file:     {} {DIM}({} vars){RESET}", env_path.display(), env_vars.len());
    } else {
        println!("{BOLD}{CYAN}[dry-run]{RESET} Env file:     {DIM}(none){RESET}");
    }

    println!("{BOLD}{CYAN}[dry-run]{RESET} Endpoint ID:  {}", config.endpoint_id);

    match client.find_stack_by_name(&config.name)? {
        Some(existing) => {
            println!("{BOLD}{YELLOW}[dry-run]{RESET} Would {BOLD}update{RESET} existing stack '{BOLD}{}{RESET}' (id: {})", existing.name, existing.id);
        }
        None => {
            println!("{BOLD}{GREEN}[dry-run]{RESET} Would {BOLD}create{RESET} new stack '{BOLD}{}{RESET}'", config.name);
        }
    }
...
pub fn sync(config: &Config, client: &PortainerClient) -> Result<()> {
    ...
    match client.find_stack_by_name(&config.name)? {
        ...
        None => {
            ...
            let stack =
                client.create_stack(config.endpoint_id, &config.name, &compose_content, env_vars)?;
